...
PUBLIC_KEY_PATH = os.path.abspath(os.path.join(SCRIPT_DIR, "../build/public_key.pem"))
SIGNATURE_PATH = os.path.abspath(os.path.join(SCRIPT_DIR, "../build/signature.bin"))
...
with open(constants_path, "w") as f:
    ...
    f.write(f'EXPIRATION_DATE_UTC = datetime({expiry_date.year}, {expiry_date.month}, '
            f'{expiry_date.day}, {expiry_date.hour}, {expiry_date.minute}, '
            f'{expiry_date.second}, tzinfo=timezone.utc)\n')

print(f"constants.py successfully written to {constants_path}")